    ...
    <div class="max-w-md px-2 md:px-4 py-20 mx-auto text-gray-700">
      <div class='w-full relative items-center'>
        <div class="relative max-w-xl mx-auto px-4 md:px-12 py-6 text-center rounded-2xl border border-gray-500 my-8">
          <h1 class="text-xl font-semibold mt-4 mb-8">Register</h1>
            ...
            >
              <div class="my-4">
                ...
                <fieldset class="relative w-full border border-gray-300">
                  <input v-model="form.username" id="username" class="w-full px-6 py-4" placeholder="uniq_username" required type="username">
                </fieldset>
              </div>
              <div class="my-4">
                <label class="text-xs" for="email">Email<span class="text-red-700">*</span></label>
                <fieldset class="relative w-full border border-gray-300">
                  <input v-model="form.email" id="email" class="w-full px-6 py-4" placeholder="example@gmail.com" required type="email">
                </fieldset>
              </div>
              <div class="my-4">
          ...
          <span>Already have account ? <router-link class="text-blue-700 hover:no-underline underline" to="/login">Login</router-link></span>
        </div>
      </div>
    </div>
  </div>
...
<script setup lang="ts">
  ...
  const register = async () => {
    const response = await store.dispatch('user/REGISTER_AUTHENTICATION', JSON.parse(JSON.stringify(form)))
